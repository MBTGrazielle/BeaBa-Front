...
const atualizarItem = async () => {
  ...
  try {
    ...
    localStorage.setItem('usuarioNome', nome_usuario)
    localStorage.setItem('usuarioEmail', email)
    localStorage.setItem('usuarioMatricula', matricula)
    localStorage.setItem('usuarioAcesso', tipo_acesso)
    localStorage.setItem('usuarioArea', nome_area)
    localStorage.setItem('usuarioSquad', squad)
    localStorage.setItem('usuarioEquipe', equipe)
    localStorage.setItem('usuarioCargo', cargo)

    // Verifica se uma imagem de perfil está disponível
    ...
    const response = await fetch(
      `http://localhost:3008/adm/atualizar/${id_usuario}`,
      {
        method: "PATCH",
        body: formData,
      }
...
btnBuscarUsuario.addEventListener("click", async function () {
  ...
  try {
    if (matricula.length === 0) {
      ...
      const response = await fetch(
        `http://localhost:3008/adm/buscarUsuarios/${matricula}`
      );
...
btnExcluirUsuario.addEventListener("click", async function () {
  ...
  btnExcluirUsuario.innerHTML = 'Excluindo...'
  btnExcluirUsuario.disabled = true


  try {
    const response = await fetch(
      `http://localhost:3008/adm/deletar/${id_usuario}`,
      {
        method: "DELETE",
      }
    );
