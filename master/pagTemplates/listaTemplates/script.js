...
window.addEventListener("load", async function () {
  ...
  `;
          ...
          btnAtualizar.addEventListener("click", async () => {
            ...
            const resposta = await fetch(`http://localhost:3008/master/atualizarTemplate/${templateId}`, {
              method: 'PATCH',
              headers: {
  ...
  const buscarTemplate = document.getElementById('buscar-template');
  ...
  itemUpload.forEach((element, index) => {
    element.addEventListener("click", async (event) => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = `../../upload/index.html?templateId=${templateId}`;
        } else if (result.dismiss === Swal.DismissReason.cancel) {
          Swal.fire("Ação cancelada", "", "error");
        }
      });
...
async function atualizarListaTemplatesPendentes() {
  ...
  itemReprovar.forEach((element, index) => {
    element.addEventListener("click", async (event) => {
      ...
      try {
        const buscaMaster = await buscarInformacoesDoUsuario()
        let nomeMaster = buscaMaster.usuario[0].nome_usuario
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            try {
              await invalidarTemplate(templateId, email, motivo_invalidacao, nomeMaster);
              Swal.close();
...
async function inativarTemplate(template) {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3008/master/inativarTemplate/${id_template}`,
      {
        method: "PATCH",
        body: formData,
      }
...
async function pendenteTemplate(template) {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3008/master/pendenteTemplate/${id_template}`,
      {
        method: "PATCH",
        body: formData,
      }
...
const invalidarTemplate = async (templateId, email, motivo_invalidacao, nome) => {
  try {
    ...
    const response = await fetch(
      `http://localhost:3008/master/invalidarTemplate/${templateId}/${email}/${nome}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
async function visualizarTemplate(template) {
  ...
  try {
    const response = await fetch(`http://localhost:3008/master/visualizar/${id_template}`, {
      method: "GET",
    });
...
async function deletarTemplate(templateId) {
  try {
    const response = await fetch(`http://localhost:3008/master/deletarTemplate/${templateId}`, {
      method: "DELETE",
    });
...
async function deletarCampo(templateId) {
  try {
    const response = await fetch(`http://localhost:3008/master/deletarCampo/${templateId}`, {
      method: "DELETE",
    });
...
async function ativarTemplate(templateId) {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3008/master/ativarTemplate/${id_template}`,
      {
        method: "PATCH",
        body: formData,
      }
...
inputBuscaTemplate.addEventListener('input', async () => {
  ...
  let resposta = await buscarTemplateStatus(status, area_usuario);
