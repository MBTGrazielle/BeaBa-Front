...
async function cadastrarTemplate() {
  ...
  try {
    if (!formData) {
      ...
      const response = await fetch(`http://localhost:3008/master/cadastrarTemplate/${id_usuario}`, {
        method: "POST",
        headers: {
...
async function cadastrarCampos(id_template) {
  ...
  for (let index = 0; index < camposInput.length; index++) {
    ...
    try {
      const response = await fetch(`http://localhost:3008/master/cadastrarCampo/${id_template}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
async function deletarTemplate(id_template) {
  try {
    const response = await fetch(`http://localhost:3008/master/deletarTemplate/${id_template}`, {
      method: "DELETE",
    });
